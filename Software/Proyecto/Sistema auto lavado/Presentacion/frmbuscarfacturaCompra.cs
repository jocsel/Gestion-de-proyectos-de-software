...
{
    public partial class frmbuscarfacturaCompra : Form
    {

        List<Ecompra> listacompra;
        public int idproveedor, idusuario, idcompra;
        ...
        public void filtrar()
        {
            ...
            if (rbcodigo.Checked)
            {
                var lista = (from compra in listacompra
                             where compra.Idcompra.ToString().StartsWith(textBox1.Text.ToUpper())
                             select new
                             {
                                 Numventa = compra.Idcompra,
        ...
        {
            Ncompra listam = new Ncompra();
            listacompra = listam.obtenerlistventa();

            var lista = (from compra in listacompra                        
                         select new
                         {
                             Numcompra = compra.Idcompra,
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                idcompra = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["Numcompra"].Value.ToString());
                idproveedor = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["proveedor"].Value.ToString());
